...
const [purchaseQty, setPurchaseQty] = useState(""); // quantity entered
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/sweets/search?${queryParams.toString()}`, {
      headers: {
        Authorization: t,
      },
    })
  ...
  const handlePurchase = async (id, purchaseQty) => {
    console.log(id,purchaseQty)
     try {
      ...
      const response = await fetch(`${apiUrl}/api/sweets/${id}/purchase`, {
        method: "post",
        headers: {
      ...
      setSuccessMsg("🎉  Order Successful ! Your sweet will be delivered soon.");
